...
def main():
   ...
   while num > 0:
     ...
     maxindex = np.argmax(data[index,indexes])#意図的に探索終了してみる。->結果的に、複数最小値があった場合、エッジ長平均値によるノードの評価を省く方が、良解にたどり着くようだ。2-optでうまく補正できているのだろうか...？(未検証。原因不明。)
     #maxindexes = [i for i,x in enumerate(data[index,indexes]) if x == xmin] #最小値のインデックスの集合を得る
     #if len(maxindexes) > 1:
     #  index = np.argmin(dataeva[indexes[maxindexes]])
   ...
   while itr > 0 :
     ...
     while num > 0:
       ...
       worst_maxnode=np.argmax(data[int(anslist[worst_node_i])])#最長のエッジを探索
       #◆交換なし
       #交換元
       worst_maxnode_i_list=np.where(anslist-worst_maxnode==0)#array[]->intに変換は[0]を参照すればよい
       worst_maxnode_i=worst_maxnode_i_list[0]
       #print("worst_maxnode_i");#print(worst_maxnode_i)

       worst_node_i_next = worst_node_i+1

       templist[1]=anslist[worst_maxnode_i]
       if worst_maxnode_i-1 > 0 :
          templist[0]=anslist[worst_maxnode_i-1]
          tempedge0[0]=data[int(templist[0]),int(templist[1])]
       if worst_maxnode_i+1 < len(anslist)-1 :
          templist[2]=anslist[worst_maxnode_i+1]
       ...
       #交換先
       templist[4]=anslist[worst_maxnode_i]#swaped
       if worst_node_i-1 > 0 :
          templist[3]=anslist[worst_node_i_next-1]
       ...
       if(np.sum(tempedge1)>np.sum(tempedge0)):
         ...
         anslist[worst_node_i_next]=anslist[worst_maxnode_i]
         anslist[worst_maxnode_i]=tempnode
       #decrement
       num-=1
     #while num end
     print("end 2-opt")
